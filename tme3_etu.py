...
def mse(datax,datay,w):
    ...
    return (((np.dot(datax, w))-datay)**2).mean(axis=0)

def mse_g(datax,datay,w):
    if len(datax.shape) == 1: datax = datax.reshape(1, -1)
    return 2*((datax.T@datax@w)-(2*datay.T@datax))

def hinge(datax,datay,w):
    """ retourne la moyenne de l'erreur hinge """
    if len(datax.shape) == 1: datax = datax.reshape(1, -1)

    return np.array([max(0, (-datay[i]*(datax[i]@w))) for i in range(len(datax))]).mean(axis=0)
